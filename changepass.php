...
$curr_pass=$_POST['cpsw'];
$new_pass=$_POST['npsw'];
$con_pass=$_POST['cnpsw'];
$sub=$_POST['sub'];
$id=$_POST['identity'];
...
if($sub == "Save"){
    ...
    if($row['password']==$curr_pass){
        $sql1 = "UPDATE patient SET password=$new_pass WHERE pat_id=$id";
        $conn->query($sql1);
    ...
    $row=$result-> fetch_assoc();
    if($row['password']==$curr_pass){
        $sql1 = "UPDATE doctor SET password=$new_pass WHERE doc_id=$id";
        $conn->query($sql1);
        echo "Password has been changed";
    }else{
        echo "You entered Wrong Password";
    ...
    $row=$result-> fetch_assoc();
    if($row['password']==$curr_pass){
        $sql1 = "UPDATE admin SET password=$new_pass WHERE adminid=1";
        $conn->query($sql1);
        echo "Password has been changed";
    }else{
        echo "You entered Wrong Password";
