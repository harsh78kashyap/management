...
die('connect error('. mysqli_connect_errno().')'. mysqli_connect_error());
    ...
    $sql3 = "SELECT p_id FROM appoint WHERE p_id='$patid'";
    $result4 = $conn->query($sql3);
    if( $result4-> fetch_assoc()){
        echo "You can't take more than one Appointment at a time";
        die();
    }


    $sql1 = "SELECT name FROM doctor WHERE doc_id='$docid'";
    $result3 = $conn->query($sql1);
    $row = $result3-> fetch_assoc();
    $re=$row['name'];
$sql = "INSERT INTO appoint (p_id,doc_id,department,date,time,doc_name) values('$patid','$docid','$depart','$date','$time','$re')";
$conn->query($sql);
?>
Your Appointment has been Successfully Submitted.
<br> Wait for the response from your Doctor.
<br>You can see Your Appointment Status in your Profile.
<?php
$conn -> close();
}
