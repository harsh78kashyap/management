...
if(empty($_POST['fromdoc'])){
    echo "You have not selected any button";
    die();
}
$id = $_POST['fromdoc'];
$sub=$_POST['sub'];
...
if($sub =="Cancel appointment"){
    $N = count($id);
    for($i=0;$i<$N;$i++){
    $sql = "DELETE FROM appoint WHERE p_id=$id[$i]";
    $conn-> query($sql);
    $s = "UPDATE oldappoint SET message='$mess' WHERE pid=$id[$i]";
    $conn -> query($s);
    }
    ...
    $N=count($id);
    for($i=0;$i<$N;$i++){
        $sql = "UPDATE appoint SET status='Confirmed' WHERE p_id=$id[$i]";
        $conn-> query($sql);
    }
    echo "appointment confiremed";
}
